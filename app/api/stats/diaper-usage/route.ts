...
import { getDiaperUsageStats } from "@/repositories/excretionEventRepository";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = await getDiaperUsageStats(babyId, from, to);
    return NextResponse.json({ data: stats });
  } catch (err) {
    console.error("[api/stats/diaper-usage] GET error:", err);
