...
const PRESET_PEE: Partial<FormState> = {
  peeAmount: "medium",
  peeColor: "light_yellow",
};

const PRESET_POOP: Partial<FormState> = {
  poopColor: "yellow",
  poopTexture: "soft",
  poopAmount: "medium",
};
...
const PRESETS: Preset[] = [
  {
    emoji: <DropletsIcon size={20} />,
    label: "ฉี่ปกติ",
    values: { type: "pee", diaperUsed: true, ...PRESET_PEE },
  },
  {
    emoji: <CirclePileIcon size={20} />,
    label: "อึปกติ",
    values: { type: "poop", diaperUsed: true, ...PRESET_POOP },
  },
  {
    emoji: <SproutIcon size={20} />,
    label: "ทั้งคู่ปกติ",
    values: { type: "both", diaperUsed: true, ...PRESET_PEE, ...PRESET_POOP },
  },
];
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {options.map((o) => (
        ...
        >
          {colorDot && o.dot && (
            <span className={`inline-block w-3 h-3 rounded-full ${o.dot}`} />
          )}
          {o.label}
        </button>
      ))}
...
export function AddExcretionEventModal({
  idToken,
  babyId,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    const payload: CreateExcretionEventPayload = {
      ...
      pee:
        form.type !== "poop"
          ? {
              amount: form.peeAmount ?? undefined,
              color: form.peeColor ?? undefined,
            }
          : null,
      poop:
        form.type !== "pee"
          ? {
              color: form.poopColor ?? undefined,
              texture: form.poopTexture ?? undefined,
              amount: form.poopAmount ?? undefined,
            }
          : null,
      smell: form.smell,
      rash: form.rash || null,
    ...
    const result = await apiPost("/api/excretion-event", idToken, payload);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-3 shrink-0">
          <h2 className="text-lg font-bold text-gray-900">บันทึกการขับถ่าย</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-5 pb-4 space-y-5">
          ...
          {/* ── Step 2: Diaper toggle ── */}
          <div>
            <SectionLabel>ใส่ผ้าอ้อม</SectionLabel>
            <div className="flex gap-2">
              <Toggle
                checked={form.diaperUsed}
                onChange={(v) => set("diaperUsed", v)}
                label="ใส่ผ้าอ้อม"
              />
            </div>
          </div>
          ...
          {showPee && (
            <div className="space-y-3 p-3 rounded-2xl bg-blue-50/50 border border-blue-100">
              ...
              <div>
                ...
                <ChipGroup
                  options={PEE_AMOUNTS}
                  value={form.peeAmount}
                  onChange={(v) => set("peeAmount", v)}
                />
              </div>
          ...
          {showPoop && (
            <div className="space-y-3 p-3 rounded-2xl bg-amber-50/50 border border-amber-100">
              ...
              <div>
                ...
                <ChipGroup
                  options={POOP_COLORS}
                  value={form.poopColor}
                  onChange={(v) => set("poopColor", v)}
                  colorDot
                />
              </div>
              ...
              <div>
                ...
                <ChipGroup
                  options={POOP_TEXTURES}
                  value={form.poopTexture}
                  onChange={(v) => set("poopTexture", v)}
                />
              </div>
              ...
              <div>
                <SectionLabel>ปริมาณ</SectionLabel>
                <ChipGroup
                  options={POOP_AMOUNTS}
                  value={form.poopAmount}
                  onChange={(v) => set("poopAmount", v)}
                />
              </div>
            </div>
          )}

          {/* ── Step 4: Shared details ── */}
        ...
        <div className="px-5 pb-6 pt-3 flex gap-2 shrink-0 border-t border-gray-100">
          ...
          >
            {submitting ? (
              ...
              <>
                <SaveIcon size={16} /> บันทึกการขับถ่าย
              </>
            )}
          </button>
        </div>
